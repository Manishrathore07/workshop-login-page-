...
def workshop_public_stats(request):
    try:
        if Workshop.objects.count() < 5:
            ...
            wt_objs = []
            for t_name in types:
                wt, _ = WorkshopType.objects.get_or_create(name=t_name)
                wt_objs.append(wt)

            u_coord, _ = User.objects.get_or_create(username='coord1', defaults={'email': 'coord1@fossee.in'})
            Profile.objects.get_or_create(user=u_coord, defaults={'position': 'coordinator', 'institute': 'IIT Bombay', 'state': 'Maharashtra', 'is_email_verified': True})
            
            u_inst, _ = User.objects.get_or_create(username='inst1', defaults={'email': 'inst1@fossee.in'})
            Profile.objects.get_or_create(user=u_inst, defaults={'position': 'instructor', 'institute': 'FOSSEE', 'state': 'Karnataka', 'is_email_verified': True})

            states = ['Maharashtra', 'Karnataka', 'Delhi', 'Gujarat', 'Tamil Nadu']
            
            # Create a variety of workshops
            for i in range(15):
                wt = random.choice(wt_objs)
                # Ensure we have profiles for different states for stats
                state = random.choice(states)
                
                # Mock a coordinator for this state
                u_c, _ = User.objects.get_or_create(username=f'user_{state.lower()}', defaults={'email': f'{state.lower()}@fossee.in'})
                Profile.objects.get_or_create(user=u_c, defaults={'position': 'coordinator', 'institute': f'University of {state}', 'state': state, 'is_email_verified': True})
